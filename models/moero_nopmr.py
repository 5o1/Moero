...
class CsmBlock(nn.Module):
    ...
    def forward(self, masked_kspace: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        csm, embedding = self.model(masked_image)
        csm = self.norm.pad_adjoint(csm)
        embedding = rearrange(embedding, "(b coil) ref c h w -> b ref coil c h w", b=masked_kspace.size(0), coil=masked_kspace.size(3)).mean(dim=2)  # b ref c h w
...
class SenseBlock(nn.Module):
    ...
    def forward(
        ...
        # Model forward pass
        model_term, embedding = self.model(model_input)
...
class GateBlock(nn.Module):
    def __init__(self, nav: nn.Module):
        super().__init__()
        self.nav = nav
        self.norm = Format4Unet2d(ndownsample=0, is_resize=False, is_pad=False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: b ref c h w, complex tensor
        x = x[:, x.size(1) // 2, ...]  # Take the central slice
        x = self.norm(x)
        route_weights, route_mask, route_idx  = self.nav(x)
        return route_weights, route_mask, route_idx 


class MoeroNoPmr(nn.Module):
    """
    ...
    def __init__(
        ...
        self.branchnavs = nn.ModuleList([
            GateBlock(
                branchnav_class(
                    in_channels = 16 if icascade == 0 else 80, # hardcoded input channels for nav
    ...
    def forward(
            ...
            for _ in range(self.moe_n_recurrent[cascade_idx]): # Recurrent times
                try:
                    ...
                    for expert_idx in experts_indices:
                        ...
                        img_pred_expert, embedding_expert = expert(
                        ...
                        # img_pred_dense.index_add_(0, batch_indices, img_pred_expert * route_weights[batch_indices, expert_idx].view(-1, *([1]* (img_pred_expert.ndim - 1))))
                        img_pred_dense, embedding_dense = map(
                            ...
                            [img_pred_dense, embedding_dense],
                            [img_pred_expert, embedding_expert]
                        )
        ...
        register_extra_output(self, "route_path", torch.as_tensor(route_path))
